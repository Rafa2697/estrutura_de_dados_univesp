...
{
 public:  
  SearchTree() { root = NULL;  } // Construtor 
  ~SearchTree(){ destroyTree(root); } // Destrutor
  bool isEmpty() const;
  bool isFull() const;
 ...
 private:
  ...
  void retrieveAluno(NodeType* tree,
		     Aluno& item,
		     bool& found) const;
  void insertAluno(NodeType*& tree, Aluno item); // Função privada que insere um item na árvore
